...
class Player():
	...
	def _get_raise_amount(self, to_call_amount):
		...
		if raise_amount < 2*to_call_amount:
			...
			return self._get_raise_amount(to_call_amount)
	...
	def decision(self, to_call_amount):
		...
		if decision == '3':
			raise_amount = self._get_raise_amount(to_call_amount)
...
class Computer:
	...
	def get_odds_of_winning(self):
		...
		# print(sum(list(calculator.scores.values())[0])/len(list(calculator.scores.values())[0]))
